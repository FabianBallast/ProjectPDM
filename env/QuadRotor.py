...
class Quadrotor(core.Env):
    ...
    def step(self, cmd_rotor_speeds):
        ...
        action = [w1, w2, w3, w4]
        '''
        rotor_speeds = np.minimum(np.maximum(cmd_rotor_speeds, self.rotor_speed_min), self.rotor_speed_max)
        ...
        # print(f"sdot {s_dot_fn(0, self.state)}")
        '''
    ...
    def _s_dot_fn(self, t, s, u1, u2):
        ...
        # print(F)
        v_dot = 1/self.mass * (F + self.weight)

        # Orientation derivative.
        q_dot = quat_dot(state['q'], state['w'])

        # Angular velocity derivative. page 26 Equation 4
        w_dot = self.inv_inertia @ (u2 - Quadrotor.hat_map(state['w'])@( self.inertia@state['w']))

        # Pack into vector of derivatives.
        s_dot = np.zeros((13,))
        s_dot[0:3] = x_dot
        s_dot[3:6] = v_dot.reshape((3))
        s_dot[6:10] = q_dot
        s_dot[10:13] = w_dot
    ...
    @classmethod
    def _quat2rot(cls, state):
        """
        Return rotation matrix from quaterinion
        """
        q = state['q'] / np.sqrt(np.sum(state['q']**2))
        ...
        return np.eye(3) + 2 * qahat @ qahat + 2 * q[0] * qahat